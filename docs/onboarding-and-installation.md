...
- **OpenAI API:** the user supplies an API key and model. The desktop app stores the
  ...
  facility; the backend receives the decrypted value only in its process environment.
...
- Secure storage unavailable: refuse to store a key and offer process environment variables or task-package mode.
