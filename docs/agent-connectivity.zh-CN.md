...
Codex App Server 无法握手、会话异常结束或没有返回可读结果时，IntDog 会关闭并移除该坏会话，然后仅重试一次同一份、已经验证过的 Codex CLI。对话栏和真实连接测试都会明确显示“本机 CLI 回退”；该次调用不再保留失效的 App Server 会话 ID。IntDog 不会自动改用 OpenAI API、其他 Agent 或其他付费来源。

IntDog 使用 Agent 提供的结构化 App Server、ACP 或无头 CLI 接口，不内嵌终端模拟器。这样可以直接判断请求完成、读取结构化事件，并在 Windows、macOS 和 Linux 使用同一套发行架构。IntDog 仍然不会接管已经打开的 Agent 桌面窗口。
