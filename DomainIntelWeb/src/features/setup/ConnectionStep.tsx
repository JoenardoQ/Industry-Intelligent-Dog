...
export default function ConnectionStep({setup,selected,setSelected,onBack,onNext,onRefresh,onUseSelected}:{
  ...
  const chooseAgent=async()=>{
    ...
    try{
      ...
      const found=selectedPath
        ...
        : discovery.items.find(item=>item.id===selected&&item.installed&&item.execution_level==='direct')
          ||discovery.items.find(item=>item.installed&&item.execution_level==='direct')
      if(!found||found.id.startsWith('selected-'))throw new Error(selectedPath?'无法识别这个命令；请选择受支持 Agent 的 CLI 可执行文件':'没有在当前系统 PATH 中发现可直接连接的 Agent')
  ...
  return <section className="setup-step" aria-labelledby="connection-title">
    ...
    <div className="agent-options">
      ...
      {setup.agents.map(item=>{const current=status?.id===item.id?status:item;const label=current.ready?'可以直接使用':current.installed?(current.authenticated===false?'需要登录':'需要检测'):'未安装';return <label key={item.id} className={selected===item.id?'selected':''}><input type="radio" name="provider" checked={selected===item.id} onChange={()=>setSelected(item.id)}/><span><b>{item.name}</b><small>{item.note}</small><code>{current.executable||item.commands.join(' / ')}</code>{status?.id===item.id&&<small>{status.version} · {status.detail}</small>}</span><em className={current.ready?'ready':'missing'}>{label}</em>{item.docs_url&&<a href={item.docs_url} target="_blank" rel="noreferrer" aria-label={`${item.name} 文档`}><ExternalLink/></a>}</label>})}
    ...
    <section className="agent-connection-tools" aria-label="本机 Agent 连接工具"><div><Cable/><span><strong>本机 Agent 没被识别？</strong><small>{bridge?'请选择 codex.exe、codex.cmd、claude 等 CLI 命令文件。IntDog 会验证身份、版本和登录状态。':'源码模式会重新扫描当前系统 PATH，并验证 Agent 的身份、版本和登录状态。'}</small></span></div><div className="section-actions"><button className="button secondary" onClick={()=>void chooseAgent()} disabled={agentBusy}>{agentBusy?'正在检测…':bridge?'选择已安装的 Agent':'自动检测本机 Agent'}</button>{profileId&&<button className="button secondary" onClick={()=>void testConnection()} disabled={agentBusy||!agentReady}><PlugZap/>测试真实连接</button>}{profileId&&<button className="button tertiary" onClick={()=>void restoreAutomatic()} disabled={agentBusy}><RotateCcw/>恢复自动检测</button>}</div>{probe&&<p className={probe.ready?'connection-result ready':'connection-result missing'}>{probe.detail} · {probe.latency_ms} ms</p>}</section>
