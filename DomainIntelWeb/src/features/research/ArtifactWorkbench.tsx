...
import { api, type GenerateResult, type ProductItem } from '../../api'
import { ChainGraph, type Toast } from '../shared'
...
export default function ArtifactWorkbench({industry,artifacts,notify}:{industry:string;artifacts:ProductItem[];notify:(value:Toast)=>void}) {
  const [busy,setBusy]=useState('');const [event,setEvent]=useState('')
  ...
  const run=async(action:string,kind='',eventText='')=>{const id=`${action}:${kind}`;setBusy(id);try{const selected=localStorage.getItem('intdog.provider')||'taskpack';const taskpack=selected==='taskpack';const result=await api<GenerateResult>(`/industries/${industry}/generate`,{method:'POST',body:JSON.stringify({action,kind,event:eventText,provider:taskpack?'':selected,execution_mode:taskpack?'taskpack':'direct',pipeline_mode:'generate'})});notify({kind:'ok',text:`任务已进入队列 · ${result.run_id.slice(0,12)}`});location.hash='/jobs'}catch(reason){notify({kind:'error',text:String(reason)})}finally{setBusy('')}}
