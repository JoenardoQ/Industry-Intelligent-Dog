...
export default function ProductsPage({industry,notify}:{industry:string;notify:(value:Toast)=>void}) {
  const [data,setData]=useState<ProductsPayload|null>(null);const [history,setHistory]=useState<HistoryCoveragePayload|null>(null);const [active,setActive]=useState<ProductItem|null>(null);const [busy,setBusy]=useState('');const [runId,setRunId]=useState('');const [error,setError]=useState('');const [historyError,setHistoryError]=useState('');const productRequest=useRef(0);const historyRequest=useRef(0)
  const loadProducts=useCallback(async()=>{const request=++productRequest.current;try{const products=await api<ProductsPayload>(`/industries/${industry}/products`);if(request!==productRequest.current)return;setData(products);const all=[...Object.values(products.periodic).flat(),...products.reports,...products.deep_reports,...products.impacts];setActive(current=>current&&all.some(item=>item.id===current.id)?current:all[0]||null);setError('')}catch(reason){if(request!==productRequest.current)return;setError(String(reason));notify({kind:'error',text:String(reason)})}},[industry,notify])
  const loadHistory=useCallback(async()=>{const request=++historyRequest.current;setHistoryError('');try{const coverage=await api<HistoryCoveragePayload>(`/industries/${industry}/history`);if(request===historyRequest.current)setHistory(coverage)}catch(reason){if(request===historyRequest.current)setHistoryError(String(reason))}},[industry])
  useEffect(()=>{setData(null);setHistory(null);setActive(null);setError('');setHistoryError('');void loadProducts();void loadHistory();return()=>{productRequest.current+=1;historyRequest.current+=1}},[loadProducts,loadHistory])
  ...
  return <><Header eyebrow="RESEARCH PRODUCTS" title="研究产物" body="六个周期直接生成；每份产物在同一安全阅读器中显示状态、元数据、引用、限制与可视化。"/>
    ...
    <RunFeedback runId={runId} onTerminal={()=>{void loadProducts();void loadHistory()}}/>
    {historyError&&<section className="section-card inline-recovery" aria-label="历史覆盖读取失败"><div><strong>历史覆盖暂时无法读取</strong><p className="subtle">已有研究产物仍可正常阅读。</p></div><button className="button secondary" onClick={()=>void loadHistory()}>重试历史覆盖</button></section>}
