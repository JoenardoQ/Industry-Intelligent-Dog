...
describe('first-run and industry overview loop', () => {
  ...
  it('diagnoses four connection modes and keeps bootstrap gates in the wizard until confirmed', async () => {
    ...
    expect(await screen.findByText('已保存来源')).toBeInTheDocument()
    expect(await screen.findByText('8')).toBeInTheDocument()
    expect(screen.getByRole('progressbar')).not.toHaveAttribute('value')
    expect(screen.getByText(/进度暂不可估算/)).toBeInTheDocument()
    expect(screen.getByText('已保存产业链节点').parentElement).toHaveTextContent('3')
    expect(screen.getByText('已保存实体').parentElement).toHaveTextContent('12')
