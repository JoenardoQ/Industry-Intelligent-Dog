...
机器进度采用有边界的逐行协议。即使一条事件被拆成多个 stdout 数据块，阶段、检查点和安全错误类别也不能丢失。首次初始化成功后立即展示候选覆盖、已复核证据覆盖、剩余缺口和通往下一步的直接链接；后续扩展有预算上限并由用户触发。

用户工作流以结果而不是任务队列为中心。初始化连接失败后，修复连接必须回到同一行业和原任务检查点。前台任务进入终态时，发起页面直接刷新本地数据，或显示产物与已保留的部分结果。任务中心统一翻译内部状态和阶段，默认选择有用任务，并把结果送回所属页面。辅助模块独立失败：覆盖或历史统计不可用时，已经存在的情报与报告仍能阅读。
...
`VERSION` 是产品版本的唯一权威。Web、Desktop、安装包名称、MCP 报告版本和发行参数必须与其一致，才能进入打包。由于产品的 `-test.N` 写法不符合 PEP 440，Python 发行元数据使用明确映射 `.devN`。
