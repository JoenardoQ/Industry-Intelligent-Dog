...
Beta 状态：当前工作树必须通过相同 revision 的原生门槛与外部门槛后，才可公开分发。
...
正式桌面入口采用 Electron 窗口承载 React 工作台；冻结 sidecar 在随机 localhost 端口提供
受会话保护的 API。源码 Chrome app-mode 启动器仅作为 developer-only 兼容路径，明确不进入
安装包。4.0 起只有 React 这一套产品 UI；旧 Tk 工作台已经移除。
...
桌面端密钥只进入操作系统凭据存储，并经一次性匿名管道传给 sidecar；开发者 CLI 可显式使用
当前进程环境变量。密钥不写入受版本控制的配置。非本机模型端点必须使用 HTTPS；本地 API
只监听 localhost，不提供局域网分享模式。
...
“无模型”有两种不同结果：任务包交接不会产生完成的研究；`NOM-01` 公开免凭据采集只有在真实
来源、文档、实体与产业链 oracle 全部通过后才算最小有用流程。当前工作树尚未关闭这个联网外部门槛。
...
后台计划必须由用户在“系统状态”中主动启用：Windows 使用任务计划程序，macOS 使用用户
LaunchAgent，Linux 使用用户 systemd timer。卸载前应先在 App 中撤销/停用后台权限；关闭窗口
不会自动扩大联网或模型权限。Provider Key 由操作系统安全存储加密，并经一次性匿名管道传给
sidecar，不进入子进程环境变量。卸载只移除应用，用户数据默认保留；永久删除前应先备份。
