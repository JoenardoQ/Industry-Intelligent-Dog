...
5. 来源发现后任务会暂停等待复核。进入**信息源**，阅读候选官网的关于或法律声明页，填写发布者身份证据网址、所属机构证据网址、机构名称及复核说明，再点击**采用**。IntDog 会独立检查来源网址能否安全访问；无法抓取的来源可保留为**人工阅读**。
6. 采用至少 15 个已核验来源，覆盖 10 个域名、6 类来源，其中至少 5 个是一手来源。点击**继续生成产业链与实体**，系统依次生成草稿；任何阶段未通过门槛都会停止下游工作。重试会复用已完成阶段，但已审查的来源集合发生变化后会重新生成。
7. 检查产业链顺序、实体覆盖、引用及剩余缺口，再开始每日采集或研究产物生成。草稿结构完整不代表内容已成为核实事实。
...
首次引导完成后，随时点击顶部“连接设置”更换 Agent、重新检测、测试连接或恢复自动检测。系统状态中的全局默认适用于仍在继承的行业；行业自定义不会被全局修改覆盖。GUI 已安装或正在运行并不代表可以直接调用；没有稳定非交互 CLI 的 Agent 只能通过 MCP 或任务包交接。
...
论文采集同时覆盖成熟主题与前沿候选，用于发现可能形成新产业方向、但尚未得到行业验证的技术；这些内容保持 `candidate`，不会自动变成事实。采集预算限制工作量；条目更多不代表覆盖完整。
...
- `partial` 表示产物未通过质量检查或任务包未完整完成，应查看质量详情。`execute-tasks` 分别统计草稿、部分产物、跳过、失败和未开始项，未完整完成时退出码为 2。命令输出的运行清单路径保留逐项结果与产物快照路径，Provider 失败后也可查看；不会自动重试失败调用。通过检查的草稿仍需复核。
